...
proto.connect = function( maze ) {
  this.connectOrientation( maze, 'noon' );
  this.connectOrientation( maze, 'three' );
  this.connectOrientation( maze, 'six' );
  this.connectOrientation( maze, 'nine' );
};

proto.connectOrientation = function( maze, orientation ) {
  var line = this[ orientation ];
  maze.connect( this, orientation, line.a );
  maze.connect( this, orientation, line.b );
};
...
proto.connect = function( maze ) {
  maze.connect( this, 'noon', this.a );
  maze.connect( this, 'noon', this.b );
  maze.connect( this, 'three', this.a );
  maze.connect( this, 'three', this.b );
  maze.connect( this, 'six', this.a );
  maze.connect( this, 'six', this.b );
  maze.connect( this, 'nine', this.a );
  maze.connect( this, 'nine', this.b );
};
...
proto.connectOrientation = function( maze, orientation ) {
  var line = this[ orientation ];
  if ( maze.getIsPegOut( line.a ) || maze.getIsPegOut( line.b ) ) {
    return;
  }
  maze.connect( this, orientation, line.a );
  maze.connect( this, orientation, line.b );
...
cub.render = function( ctx, mazeCenter, gridSize, angle, isHovered ) {
  ...
  // function eye( direction ) {
  //   ctx.beginPath();
  //   ctx.arc( gridSize * 0.25 * direction, gridSize * 0.05, gridSize * 0.1, Math.PI, Math.PI*2 );
  //   ctx.fill();
  //   ctx.closePath();
  // }
  ...
  // // nose
  // ctx.beginPath();
  // ctx.arc( 0, gridSize * 0.15, gridSize * 0.1, 0, Math.PI );
  // ctx.fill();
  // ctx.closePath();
...
/* globals FlyWheel, FreeSegment, FixedSegment, PivotSegment, cub */
...
proto.loadText = function( text ) {
  ...
  if ( sections.length > 1 ) {
    var frontMatter = getFrontMatter( sections[0] );
  }
  // set instruction
...
proto.connect = function( segment, orientation, position ) {
  // flatten the key
  var key = orientation + ':' + position.x + ',' + position.y;
...
proto.render = function( ctx, center, gridSize, angle ) {
  ...
  // fixed segments
  this.fixedSegments.forEach( function( item ) {
    item.render( ctx, center, gridSize );
  });
  // rotation
  ctx.rotate( angle );
...
completedLevels.forEach( function( id ) {
  levelList.querySelector('[data-id="' + id + '"]').classList.add('did-complete');
});
